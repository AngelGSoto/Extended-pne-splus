...
Abstract: We present a catalogue of white dwarf candidates selected from the second data release of Gaia (DR2). We used a sample of spectroscopically 
confirmed white dwarfs from the Sloan Digital Sky Survey (SDSS) to map the entire space spanned by these objects in the Gaia Hertzsprung-Russell diagram. 
We then defined a set of cuts in absolute magnitude, colour, and a number of Gaia quality flags to remove the majority of contaminating objects. Finally, 
we adopt a method analogous to the one presented in our earlier SDSS photometric catalogues to calculate a probability of being a white dwarf (PWD) for all 
Gaia sources which passed the initial selection. The final catalogue is composed of 486641 stars with calculated PWD from which it is possible to select a 
sample of∼260000 high-confidence white dwarf candidates in the magnitude range 8<G<21. By comparing this catalogue with a sample of SDSS white dwarf candidates 
we estimate an upper limit in completeness of 85 per cent for white dwarfs with G≤20mag and Teff>7000K, at high Galactic latitudes (|b|>20deg). However, 
the completeness drops at low Galactic latitudes, and the magnitude limit of the catalogue varies significantly across the sky as a function of Gaia's 
scanning law. We also provide the list of objects within our sample with available SDSS spectroscopy. 
...
I cross-matched the sample of bluer WD with MAIN3.1 catalog. The file resulting was: =MAIN31-WD-gaia=
 - I putted them in my color-color diagrams with:
    : python ../../varios-programas/color-diagrams-splusdr2_automatic-v2.py MAIN31-WD-gaia

 - Making the rgb images using R, F660 and I filters:
     : python ../../../../varios-programas/rgb_image-v2.py HYDRA-0054_I_00056653_swp-crop HYDRA-0054_F660_00056653_swp-crop HYDRA-0054_R_00056653_swp-crop --vmin_r -0.5 --vmax_r 10.160 --vmin_g -0.01 --vmax_g 1.0  --vmin_b -0.2 --vmax_b 10.160 --position HYDRA-0054-000056653-position --debug

** Automatizando los programas:

1. Escribí un programa para escribir las DS9 region (Write DS9 region.ipynb, basados en los programas del profesor Will) de cada objeto individual que se encuentran en una determinada tabla,
 pero no funcionó del todo. Pero lo que sÍ hace es que escribe un archivo.reg de cada objeto (pero en cada archivo escribe las posicisiones de cada objeto). Lo automtizÉ mÁs, ahora
encuentra la carpeta donde esta las images de cada objeto y ahÍ guarda el archivo .reg.

 + Improved 
   : Write DS9 region.ipynb
...
3. Automatizé aún mas el progama que corta las imágenes (/varios-programas/cut-images-fits.py), sólo basta correr el comando en la terminal: 
   : for D in ./HYDRA*; do if [ -d "$D" ]; then cd "$D"; for f in *.fits; for e in *.reg; do python ../../../../varios-programas/cut-images-fits.py ${f%.fits} --position ${e%.reg} --debug; done; cd ..; fi; done

4. Pare correr el programa que extrae la emission de Halpha, sólo hay que hacer:
